...
namespace HiarcSDKIntegrationTest.Tests
{
    public class GroupTests : HiarcTestBase
    {
        ...
        public async void GroupCRUD()
        {
            ...
            Assert.Equal(g1, fetchedGroup, new SdkEntityComparer());

            var newName = "New Name";
            var newDescription = "New description";
            var updateRequest = new HiarcSDK.Model.UpdateGroupRequest { Name=newName, Description=newDescription };
            ...
            updateRequest = new HiarcSDK.Model.UpdateGroupRequest { Key="new key", Name=newName, Description=newDescription };
        ...
        public async void CreateGroupWithMetadata()
        {
            ...
            Assert.Equal(g1, fetchedGroup, new SdkEntityComparer());
            AssertMetadataSdk(md, fetchedGroup.Metadata);
        }
        ...
        public async void UpdateMetadata()
        {
            ...
            var request = new HiarcSDK.Model.UpdateGroupRequest
            {
                Metadata = updatedMD
            };

            var updatedGroup = await _hiarc.UpdateGroup(g1.Key, request);
            AssertMetadataSdk(updatedMD, updatedGroup.Metadata);
        }
        ...
        public async void NullOutMetadata()
        {
            ...
            request = new HiarcSDK.Model.UpdateGroupRequest
            {
                Metadata = updatedMD
            };
        ...
        public async void FindGroups()
        {
            ...
            var request = new HiarcSDK.Model.FindGroupsRequest { Query = query };
            var foundGroups = await _hiarc.FindGroups(request);
            Assert.Single(foundGroups);
            Assert.Equal(g1, foundGroups[0], new SdkEntityComparer());
        }
    }
}
