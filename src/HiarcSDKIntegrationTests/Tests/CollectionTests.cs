...
namespace HiarcSDKIntegrationTest.Tests
{
    public class CollectionTests : HiarcTestBase
    {
        ...
        public async void CollectionCRUD()
        {
            ...
            Assert.Equal(c1, fetchedCollection, new SdkEntityComparer());

            var newName = "New Name";
            var newDescription = "New description";
            var updateRequest = new HiarcSDK.Model.UpdateCollectionRequest { Name = newName, Description = newDescription };
            ...
            updateRequest = new HiarcSDK.Model.UpdateCollectionRequest { Key = "new key", Name = newName, Description = newDescription };
        ...
        public async void CreateAccessAndUpdateCollectionAsUser()
        {
            ...
            await _hiarc.AddUserToCollection(c1.Key, u2.Key, HiarcSDK.Model.AccessLevel.READ_ONLY);
            ...
            updateRequest = new HiarcSDK.Model.UpdateCollectionRequest { Name = newName, Description = newDescription };
            await Assert.ThrowsAnyAsync<Exception>(async () => await _hiarc.UpdateCollection(c1.Key, updateRequest, asUserKey: u2.Key));

            await _hiarc.AddUserToCollection(c1.Key, u3.Key, HiarcSDK.Model.AccessLevel.READ_WRITE);
            newName = "New Name 3";
        ...
        public async void HierarchyTests()
        {
            ...
            await _hiarc.AddUserToCollection(c2.Key, u1.Key, HiarcSDK.Model.AccessLevel.READ_ONLY);
            ...
            Assert.Equal(c2, fetchedCollection, new SdkEntityComparer());

            fetchedCollection = await _hiarc.GetCollection(c3.Key, asUserKey: u1.Key);
            Assert.Equal(c3, fetchedCollection, new SdkEntityComparer());

            fetchedCollection = await _hiarc.GetCollection(c4.Key, asUserKey: u1.Key);
            Assert.Equal(c4, fetchedCollection, new SdkEntityComparer());

            fetchedCollection = await _hiarc.GetCollection(c5.Key, asUserKey: u1.Key);
            Assert.Equal(c5, fetchedCollection, new SdkEntityComparer());
        ...
        public async void GetChildCollections()
        {
            ...
            Assert.Contains(c2, children, new SdkEntityComparer());
            Assert.Contains(c3, children, new SdkEntityComparer());
            Assert.Contains(c4, children, new SdkEntityComparer());
            Assert.DoesNotContain(c5, children, new SdkEntityComparer());
            Assert.DoesNotContain(c6, children, new SdkEntityComparer());
        }
        ...
        public async void GetItems()
        {
            ...
            Assert.Contains(f1, files, new SdkEntityComparer());
            Assert.Contains(f2, files, new SdkEntityComparer());
            Assert.Contains(f3, files, new SdkEntityComparer());
            Assert.DoesNotContain(f4, files, new SdkEntityComparer());
            Assert.DoesNotContain(f5, files, new SdkEntityComparer());
            ...
            await _hiarc.DeleteFile(f5.Key);
                ...
                public async void AddChildCollectionAsUser()
                {
                    ...
                    await _hiarc.AddUserToCollection(c1.Key,u1.Key, HiarcSDK.Model.AccessLevel.READ_ONLY);
                    await Assert.ThrowsAnyAsync<Exception>(async () => await _hiarc.AddChildCollection(c1.Key,c2.Key, asUserKey:u1.Key));

                    var u2 = await _hiarc.CreateUser();
                    await _hiarc.AddUserToCollection(c1.Key,u2.Key, HiarcSDK.Model.AccessLevel.READ_WRITE);
                ...
                public async void DeleteWithFile()
                {
                    ...
                    Assert.Equal(f1, result, new SdkEntityComparer());
                ...
                public async void CreateCollectionWithMetadata()
                {
                    ...
                    Assert.Equal(c1, fetchedCollection, new SdkEntityComparer());
                    AssertMetadataSdk(md, fetchedCollection.Metadata);
                }
                ...
                public async void UpdateMetadata()
                {
                    ...
                    var request = new HiarcSDK.Model.UpdateCollectionRequest
                    {
                        Metadata = updatedMD
                    };

                    var updatedCollection = await _hiarc.UpdateCollection(c1.Key, request);
                    AssertMetadataSdk(updatedMD, updatedCollection.Metadata);
                }
                ...
                public async void NullOutMetadata()
                {
                    ...
                    request = new HiarcSDK.Model.UpdateCollectionRequest
                    {
                        Metadata = updatedMD
                    };
                ...
                public async void FindCollections()
                {
                    ...
                    var request = new HiarcSDK.Model.FindCollectionsRequest { Query = query };
                    var foundCollections = await _hiarc.FindCollections(request);
                    Assert.Single(foundCollections);
                    Assert.Equal(c1, foundCollections[0], new SdkEntityComparer());
                }
    }
}
