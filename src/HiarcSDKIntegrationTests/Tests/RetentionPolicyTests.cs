...
namespace HiarcSDKIntegrationTest.Tests
{
    public class RetentionPolicyTests : HiarcTestBase
    {
        ...
        public async void RetentionPolicyCRUD()
        {
            ...
            Assert.Equal(rp1, fetchedRetentionPolicy, new SdkEntityComparer());
            Assert.Equal((uint)60, fetchedRetentionPolicy.Seconds);

            var newName = "New Name";
            var newDescription = "New description";
            var updateRequest = new HiarcSDK.Model.UpdateRetentionPolicyRequest { Name=newName, Description=newDescription };
            ...
            updateRequest = new HiarcSDK.Model.UpdateRetentionPolicyRequest { Key="new key", Name=newName, Description=newDescription };
        ...
        {           
            var rp1 = await _hiarc.CreateRetentionPolicy(RETENTION_PERIOD_MONTH);
            var updateRequest = new HiarcSDK.Model.UpdateRetentionPolicyRequest { Seconds=RETENTION_PERIOD_DAY };

            await Assert.ThrowsAnyAsync<Exception>(async () => await _hiarc.UpdateRetentionPolicy(rp1.Key, updateRequest));

            updateRequest = new HiarcSDK.Model.UpdateRetentionPolicyRequest { Seconds=RETENTION_PERIOD_MAX };
            var updatedRetentionPolicy = await _hiarc.UpdateRetentionPolicy(rp1.Key, updateRequest);
            Assert.Equal(RETENTION_PERIOD_MAX, updatedRetentionPolicy.Seconds);
        }
        ...
        public async void DeleteFileWithUpdatedRetentionPeriod()
        {
            ...
            var updateRequest = new HiarcSDK.Model.UpdateRetentionPolicyRequest { Seconds=10};
        ...
        public async void CreateWithMetadata()
        {
            ...
            Assert.Equal(rp1, fetchedRetentionPolicy, new SdkEntityComparer());
            AssertMetadataSdk(md, fetchedRetentionPolicy.Metadata);
        }
        ...
        public async void UpdateMetadata()
        {
            ...
            var request = new HiarcSDK.Model.UpdateRetentionPolicyRequest
            {
                Metadata = updatedMD
            };

            var updatedPolicy = await _hiarc.UpdateRetentionPolicy(rp1.Key, request);
            AssertMetadataSdk(updatedMD, updatedPolicy.Metadata);
        }
        ...
        public async void NullOutMetadata()
        {
            ...
            request = new HiarcSDK.Model.UpdateRetentionPolicyRequest
            {
                Metadata = updatedMD
            };
        ...
        public async void FindRetentionPolicies()
        {
            ...
            var request = new HiarcSDK.Model.FindRetentionPoliciesRequest { Query = query };
            var foundPolicies = await _hiarc.FindRetentionPolicies(request);
            Assert.Single(foundPolicies);
            Assert.Equal(rp1, foundPolicies[0], new SdkEntityComparer());
        }
    }
}