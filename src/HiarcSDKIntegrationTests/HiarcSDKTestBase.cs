...
namespace HiarcSDKIntegrationTest
{
    ...
    public class HiarcTestBase
    {
        ...
        public const int DEFAULT_EXPIRES_IN_SECONDS = 10;
        ...
        public void AssertMetadataSdk(Dictionary<string,object> expected, Dictionary<string,object> actual)
        {
            Assert.Equal(expected["department"], (actual["department"]));
            Assert.Equal(expected["quotaCarrying"], (actual["quotaCarrying"]));
            Assert.Equal(expected["targetRate"], (actual["targetRate"]));
            Assert.Equal(expected["level"], Convert.ToInt32(actual["level"]));
            Assert.Equal((DateTime)expected["startDate"], (actual["startDate"]));
        }
